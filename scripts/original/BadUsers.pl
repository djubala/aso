...
if ( $numArgs != 0){
	if ( $numArgs == 1 ){
		if ( _________________ ) { # si el primer argument es -p
...
@password_db= ____________; # llegir tot el fitxer de password
close FILE;

foreach $user_line (@password_db) {
	_________________; # eliminar el salt de línia
...
if ( $p == 1 ){
	...
	foreach $process_list_line (@process_list) {
		...
		@fields_proc = split(_____________ , “$process_list_line”);
